...
// Гліф займає 80% фішки замість колишніх 60%. Гліфи game-icons намальовані на сітці 512 і несуть
// набагато більше деталей, ніж рукописні 24×24, які тут стояли: при 60% заповнення тонкі елементи
// зникали на оглядовому масштабі повністю. Фішка при цьому стала МЕНШОЮ, а гліф у ній — більшим.
const ICON_GLYPH_BOX = 19;      // CSS px — у цей квадрат вписується гліф
const ICON_GLYPH_GRID = 512;    // сітка, на якій намальовано path-рядки
...
// А вниз, а не вгору, тому що над вузлом уже стоїть його номер (threat-vector-place, text-offset
...
/**
 * Точка, що безперервно їде дугою від A до Б.
 *
 * ЩО ВОНА ЗНАЧИТЬ І ЧОГО НЕ ЗНАЧИТЬ. Вона показує НАПРЯМОК ствердженого руху, те, що ланцюг живий,
 ...
 *
 * Попередня версія цього модуля свідомо цього не робила: одноразове промальовування лишало по собі
 * нерухому лінію, бо «лінія, що вічно біжить, читалася б як рух, який триває зараз». Власник
 * продукту зважив цей ризик проти того, що вектор без руху не читався взагалі — куди й звідки —
 * і вибрав рух. Межу в `CONTEXT.md` оновлено разом із цим кодом, щоб документ не описував
 * поведінку, якої вже немає.
 *
  ...
  for (const vector of vectors) {
    for (const [order, segment] of (vector.segments ?? []).entries()) {
      ...
      features.push({ type: 'Feature', id: `vt-${vector.eventId}-${order}`,
        ...
        properties: { basis: segment.basis, label: `${from.name} → ${to.name}`,
  ...
  for (const vector of vectors) {
    ...
    for (const node of vector.nodes ?? []) {
      ...
      features.push({ type: 'Feature', id: `vn-${vector.eventId}-${node.index}`,
        ...
        properties: { eventId: vector.eventId, name: shortPlace(node.name),
...
function vectorHeadCollection() {
  ...
  // Питання «ЩО по ньому летить» тепер має відповідь у русі: той самий гліф їде дугою
  // (`vectorTravelCollection`), тож фішка на голові лишається тим, чим була, — міткою останнього
  // повідомленого місця. Текстової назви класу на карті немає навмисно: одного разу її додали
  // окремим шаром, і вона перетворила три ланцюги на дюжину однакових написів.
  //
...
function addVectorLayers() {
  ...
  map.addLayer({ id: 'threat-vector-place', type: 'symbol', source: 'threat-vector-points', minzoom: 5.6, layout: {
    ...
    'text-padding': 3
    // Підпис належить ланцюгу, тож бере його колір. Теплий #ffd9c9 тут був відлунням помаранчевого
    // вектора і на червоній заливці зникав разом із ним.
  }, paint: { 'text-color': vectorColor, 'text-halo-color': '#06080c', 'text-halo-width': 1.8 } }, anchor);
  ...
  if (iconImagesReady) {
    ...
    map.addLayer({ id: 'threat-vector-class', type: 'symbol', source: 'threat-vector-heads',
      filter: ['==', ['get','kind'], 'class'], layout: {
        ...
        // Тексту тут немає навмисно — див. коментар до `chip()` у `vectorHeadCollection`.
...
function threatIconImage(threatType, tone) {
  ...
  // nonzero, а не evenodd. Рукописні гліфи, що тут стояли, малювали отвори зустрічним обходом і
  // вимагали evenodd; гліфи game-icons намальовані під типове для SVG правило nonzero, і evenodd
  // вивернув би їм кожен внутрішній контур навиворіт.
  ctx.fill(new Path2D(threatIconPaths[threatType] ?? threatIconPaths.unknown));
  ctx.restore();
  ...
  legend.innerHTML = `<summary><i class="swatch threat"></i><span class="legend-title">Загрози на карті</span><span class="legend-sum">4 стани · 10 типів</span><span class="legend-caret" aria-hidden="true">▾</span></summary>
    <div class="legend-body">
      ...
      <p class="legend-note">Гліфи типів — <a href="https://game-icons.net" rel="noopener noreferrer" target="_blank">game-icons.net</a>,
        ліцензія <a href="https://creativecommons.org/licenses/by/3.0/" rel="noopener noreferrer" target="_blank">CC BY 3.0</a>.
        Автори: Lorc, Delapouite, Quoting.</p>
