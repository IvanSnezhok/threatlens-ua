  ...
  const byName = await pool.query<{ id: string }>(
    `SELECT id FROM locations
     WHERE lower(name_uk)=lower($1)
        ...
        OR lower(name_uk) LIKE lower($1)||'%'
     ORDER BY CASE WHEN type IN ('oblast','special_city') THEN 0 ELSE 1 END LIMIT 1`,
    [normalized]
  );
  return byName.rows[0]?.id ?? null;
}

async function persistOfficialAlertSnapshot(sourceId: string, body: unknown): Promise<void> {
  ...
  try {
    ...
    await client.query(
      `UPDATE alert_source_states SET active=false,last_seen_at=now(),updated_at=now() WHERE source_id=$1`,
      [sourceId]
    );
      ...
      await client.query(
        `INSERT INTO alert_source_states(source_id,location_id,alert_type,active,provider_started_at,external_id)
         ...
         ON CONFLICT (source_id,location_id,alert_type) DO UPDATE SET
           ...
           external_id=EXCLUDED.external_id,last_seen_at=now(),updated_at=now()`,
    ...
    for (const key of affectedKeys) {
      ...
      const aggregate = await client.query<{ active: boolean; started_at: Date | null }>(
        `SELECT bool_or(active) AS active,min(provider_started_at) FILTER (WHERE active) AS started_at
      ...
      if (aggregate.rows[0]?.active && !global.rowCount) {
        const created = await client.query<{ id: string }>(
          `INSERT INTO alert_periods(location_id,alert_type,status,started_at,external_id)
           VALUES ($1,$2,'active',COALESCE($3,now()),$4) RETURNING id`,
  ...
  if (unresolved.length) {
    throw new Error(`${sourceId}: unresolved provider locations: ${unresolved.slice(0, 20).join(', ')}`);
  }
}

export async function syncOfficialAlerts(): Promise<void> {
  ...
  try {
    ...
    await persistOfficialAlertSnapshot('ukraine-alarm', await response.json());
...
export async function syncAlertsInUa(): Promise<void> {
  ...
  try {
    ...
    await persistOfficialAlertSnapshot('alerts-in-ua', await response.json());
...
export function startIngestionScheduler(log: { info: Function; error: Function }): () => void {
  let running = false;
  const run = async () => {
    if (running) return;
    running = true;
    try { await Promise.all([syncOfficialAlerts(), syncAlertsInUa()]); }
