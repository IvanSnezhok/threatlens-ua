...
export const envSchema = z.object({
  ...
  /**
   * Скільки часу дається наративу — окремо від решти, бо він єдиний у це не вкладається.
   *
   * ================================================================================================
   * Що показали дані
   * ================================================================================================
   *
   * Виміряно 16.08.2026 на `ai_runs` за три доби, surface `narrative`:
   *
   *   success  303 виклики, p50 17 870 мс, p90 19 466 мс
   *   failed   175 викликів, p50 20 004 мс, p90 20 009 мс
   *
   * Кожна відмова падає рівно на двадцяти секундах, з точністю до мілісекунд — це не модель і не
   * мережа, це наш власний `AbortSignal.timeout(AI_TIMEOUT_MS)`. Успішні відповіді впираються в ту
   * саму стіну із запасом у пів секунди. Тобто **37 % наративів убивав наш таймер**, і читач бачив
   * детермінований запасний текст замість написаного моделлю.
   *
   * Розподіл успішних цензурований стелею, тож справжній хвіст із цих даних не читається — відомо
   * лише, що він за неї виходить. Сорок пʼять секунд дають понад дворазовий запас від виміряного
   * p90 і лишаються мізерними проти п'ятнадцятихвилинної підлоги перерахунку
   * (`ANALYTICS_RECOMPUTE_FLOOR_MS`), тож повільний наратив не може накласти прохід на прохід.
   *
   * ================================================================================================
   * Чому саме наративу, а не всім
   * ================================================================================================
   *
   * Решта surface'ів у двадцять секунд вкладається з великим запасом: `shadow` — p50 близько 5 с і
   * 8 відмов на 3196 викликів, `tactics` — p50 8–10 с і одна відмова. Підняти стелю всім означало б
   * учетверо сповільнити виявлення справжнього збою там, де його зараз видно за двадцять секунд.
   *
   * Прецедент той самий, що й у `RETROSPECTIVE_GATE_TIMEOUT_MS`: бюджет належить поверхні, а не
   * клієнту, бо його диктує те, ЩО саме модель мусить встигнути, а не те, ЧИМ її викликали.
   *
   * Підлога в 20 с — це поточне глобальне значення: нижче воно перестало б бути власним бюджетом і
   * лише повторювало б спільний. Стеля в 120 с — межа, за якою наратив починає з'їдати помітну
   * частку підлоги перерахунку.
   */
  AI_NARRATIVE_TIMEOUT_MS: z.coerce.number().int().min(20_000).max(120_000).default(45_000),
  ...
  // ---- Стеля віку повідомлення для доставки ------------------------------------------------------
  // Скільки повідомлення джерела має право прожити, перш ніж воно перестає бути новиною і стає
  // архівом. Старше за цю межу класифікується, архівується й дописується в контекст локацій — і не
  // створює нічого, що видно людині: ні події на карті, ні кадру SSE, ні сповіщення в боті.
  //
  // Навіщо це поверх `historical` (рішення власника, 20.08.2026). `ingestThreat` уже глушить
  // повідомлення дозбору, старші за власне вікно валідності (`THREAT_VALIDITY_MS`, 30 хв), і це
  // правило лишається — воно суворіше. Але воно вмикається ЛИШЕ прапорцем `historical`, який ставить
  // `src/services/source-backfill.ts`, а живий шлях MTProto (`src/sources/telegram.ts`) його не
  // ставить: після реконекту teleproto віддає пачку пропущених апдейтів з їхніми СПРАВЖНІМИ датами,
  // і кожен такий годинної давнини пост ішов у бот як щойно опублікований. Ця стеля дивиться на вік
  // повідомлення, а не на те, яким шляхом воно прийшло, тож дірку видно з типу, а не з памʼяті про
  // те, який виклик який прапорець передає.
  //
  // Чому саме година. Попередження, доставлене, коли загроза вже минула, — не запізніле
  // повідомлення, а хибна тривога: воно вчить читача знецінювати наступне. Година — це межа, за якою
  // жодне повідомлення моніторингового каналу вже не описує того, що відбувається зараз.
  //
  // 0 вимикає стелю повністю й повертає поведінку до тієї, що була до цієї зміни: глушить лише
  // 30-хвилинне вікно дозбору. Стеля 1440 хв — щоб помилка в конфігурації не могла ввімкнути
  // «глушити все, що старше за добу» як «не глушити нічого».
  SOURCE_MESSAGE_MAX_DELIVERY_AGE_MINUTES: z.coerce.number().int().min(0).max(1440).default(60),

  // ---- Розбір атаки після відбою -----------------------------------------------------------------
  // Скільки повідомлень каналів має припасти на тривогу, щоб після відбою був сенс писати розбір.
  // Одне-два повідомлення за тригодинну тривогу — це не розбір атаки, а пости, які підписник уже
  // прочитав; лист заради листа вчить його гортати наступний. Три — це поріг, за яким у розборі вже
  // є що порахувати: класи, кількість, маркери наслідків.
  //
  // 0 не вимикає розбір, а робить його безумовним; вимикається він тим, що читачі не підписані на
  // відбої, і `ATTACK_DEBRIEF_ENABLED` нижче.
  ATTACK_DEBRIEF_MIN_MESSAGES: z.coerce.number().int().min(0).max(100).default(3),
  // Чи слати розбір підписникам після відбою. Вимкнений — розбір лишається доступним на вимогу
  // (команда бота) і на сторінці аналізу: вимикається розсилка, а не сам розбір.
  ATTACK_DEBRIEF_ENABLED: z.string().default('true').transform((value) => value === 'true')
}).superRefine((env, ctx) => {
...
export const APP_SETTINGS: Record<keyof AppConfig, SettingMeta> = {
  ...
  SOURCE_MESSAGE_MAX_DELIVERY_AGE_MINUTES: {
    scope: 'db_tunable', group: 'telegram', apply: 'hot',
    ui: { kind: 'number', min: 0, max: 1440, unit: 'хв' },
    applyNote: 'Читається на кожне повідомлення. Старші за цю межу лишаються в архіві й у контексті '
      + 'локацій для моделі, але не створюють події, кадру SSE чи сповіщення. 0 — вимкнути стелю.'
  },
  ATTACK_DEBRIEF_ENABLED: {
    scope: 'db_tunable', group: 'telegram', apply: 'hot', ui: { kind: 'boolean' },
    applyNote: 'Вимикає лише розсилку розбору після відбою. Сам розбір лишається доступним на вимогу '
      + 'і на сторінці аналізу атак.'
  },
  ATTACK_DEBRIEF_MIN_MESSAGES: {
    scope: 'db_tunable', group: 'telegram', apply: 'hot', ui: { kind: 'number', min: 0, max: 100 }
  },
  ...
  AI_NARRATIVE_TIMEOUT_MS: {
    scope: 'db_tunable', group: 'analytics', apply: 'hot',
    ui: { kind: 'number', min: 20_000, max: 120_000, unit: 'мс' },
    applyNote: 'Власний бюджет наративу, окремий від AI_TIMEOUT_MS. Виміряно 16.08.2026: на спільних '
      + 'двадцяти секундах наш же таймер убивав 37% наративів (175 відмов рівно на 20 004 мс проти '
      + '303 успіхів із p90 19 466 мс), і читач бачив детермінований запасний текст. Читається на '
      + 'кожному виклику, тож зміна діє з наступного наративу.'
  },
