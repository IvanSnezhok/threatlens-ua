...
describe('classifier against the hand-labelled gold corpus', () => {
  ...
  // snapshot belongs.
  //
  // `v8` (піктограма 🛵/🏍 на початку рядка називає клас `uav`, коли слово його не назвало) не
  // зрушує жодної осі з підлогою: significance P=100.0% / R=98.5%, точність класу 99.3%, locations
  // P=99.6% / R=99.2% — ті самі числа, що й у `v7`. Рухаються лише два рядки звіту: uav TP 100 → 103
  // і unknown TP 17 → 14 (P 94.4% → 93.3%, бо знаменник менший, а єдиний промах — той самий
  // `cruise_missile`). Причина — три мітки, переписані з `unknown` на `uav` разом зі зміною
  // конвенції; фікстура називає їх у `header.corrections`. Той самий код на старих мітках показав би
  // uav P=97.1% і провалив би підлогу — це мітки, що описували стару конвенцію, а не хибні
  // спрацювання, і саме тому вони переписані, а підлога — ні.
  //
