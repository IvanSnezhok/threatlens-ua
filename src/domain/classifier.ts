 ...
 *   use.
 * * `v7` — телеграфний репорт, у якому зброю не названо жодним словом.
 *
 *   Виміряно на розміченому корпусі (191 повідомлення, `src/domain/classifier-gold.test.ts`): усі
 *   тринадцять пропущених попереджень — **9.6 % усіх справжніх** — відхилялися з однієї й тієї
 *   самої причини, `not_an_assertion`, і всі тринадцять були однієї форми. «Київщина: 6 на Бровари
 *   зі сходу», «Ціль на Київ», «Швидкісна на Київ», «Курс Бородянка», «Вишгород, до вас
 *   намагаються летіти», «Дніпро увага з Заходу», «🛵Житомирщина: знову на Андрушівку - Озерне» —
 *   найкоротший і найтерміновіший різновид повідомлення, який ці канали взагалі пишуть, і рівно
 *   той, у якому зброю встановлює контекст каналу, а не слово.
 *
 *   Прочитано їх як {@link contextIndicators} з порожнім `threatTypes`: подія піднімається як
 *   `unknown`, бо назвати клас означало б дочитати його з піктограми або з попереднього поста.
 *   Кожна гілка має власний структурний замок, а не тему (див. {@link TELEGRAPHIC_TARGETING}), і
 *   всі вони, як і решта контекстних індикаторів, вмикаються лише поруч із розвʼязаним українським
 *   місцем. Три гілки, які `v6` мав би й не мав: «курс» без прийменника перед великою літерою —
 *   рівно та форма, яку коментар до {@link REPORTED_DIRECTION} називає поверненною, щойно регулярка
 *   перестане бути case-insensitive; число з прийменником перед назвою; і 🛵 як префікс рядка
 *   бюлетеня — 46 таких повідомлень у корпусі, всі до одного справжні попередження.
 *
 *   Разом із цим: загроза, названа разом із точкою пуску по той бік кордону
 *   ({@link FOREIGN_ORIGIN_THREAT}) — «Триває загроза балістики з Брянська» — більше не гаситься
 *   сторожем іноземних місць і має обсяг країни, бо саме це вона про нас і каже.
 *
 *   Результат на тому самому корпусі: significance R 90.4 % → **98.5 %** (пропусків 13 → 2),
 *   locations R 93.1 % → **99.2 %** (18 → 2), precision — без зміни, **100 %**, нуль хибних
 *   спрацювань. Єдина ціна: точність класу 100 % → 99.3 %, бо одне з новознайдених повідомлень
 *   читається як `unknown` там, де рецензент назвав `cruise_missile`. Попередження без назви класу
 *   краще за відсутнє попередження, і це та сама угода, яку вже уклали стрілковий бюлетень і
 *   «ракета без уточнення типу».
 *
 *   Повтор по бойовому архіву (знімок 01.09.2026, 32 192 повідомлення з вердиктом `v6`):
 *   **втрачено 0**, і це не спостереження, а властивість — індикатор уміє лише додавати
 *   значущість. Новознайдених — 2 631, з них **1 463 стояли в архіві як `unrecognized` або
...
/**
 * Рух, про який повідомляють УПЕРЕД, а не назад.
 *
 * Навмисно вужче за {@link GENERIC_THREAT_CONTEXT}: звідси прибрано «атаку», «удар», «збито» й
 * «тривог». Ті слова роблять операційним репортом повідомлення, у якому вже названо зброю, — і
 * рівно вони ж стоять у кожному підсумку про минулу ніч. Шаблони нижче зброї не мають ЖОДНОЇ, тож
 * єдине, що відрізняє «на Семиполки летить» від «через атаку на Київщині пошкоджено», — напрямок
 * дієслова в часі. Компасна вказівка («зі сходу») лишається: джерело каже, звідки ЗАРАЗ іде ціль.
 */
const TELEGRAPHIC_MOTION = /((?<!\p{L})лет(?:ить|ять|іти)|(?<!\p{L})йд(?:е|уть)(?!\p{L})|заход(?:ить|ять)|наближ|прямує|руха(?:ється|ються)|(?<!\p{L})курс|напрямк|(?<!\p{L})ціл(?:ь|і|ей|ям|ями)(?!\p{L})|швидкісн[а-яіїєґ]*|(?<!\p{L})увага(?!\p{L})|(?<!\p{L})з[іс]?\s+(?:сходу|заходу|півночі|півдня|південного|північного))/iu;

/**
 * Телеграфний репорт, у якому зброю не названо жодним словом.
 *
 * ================================================================================================
 * Що це за повідомлення
 * ================================================================================================
 *
 * «Київщина: 6 на Бровари зі сходу», «Ціль на Київ», «Швидкісна на Київ», «Курс Бородянка»,
 * «🛵Житомирщина: знову на Андрушівку - Озерне», «Вишгород, до вас намагаються летіти»,
 * «Дніпро увага з Заходу» — тринадцять таких повідомлень у розміченому корпусі, і ЖОДНЕ з них
 * класифікатор не читав: усі тринадцять відхилялися як `not_an_assertion`, бо в них немає ні назви
 ...
 * ================================================================================================
 *
 * Усі вони, як і решта {@link contextIndicators}, вмикаються ЛИШЕ поруч із розвʼязаним українським
 * місцем. Понад це кожна гілка має власну структурну вимогу, і жодна не покладається на тему:
 *
 *   * `Курс Бородянка` — регулярка БЕЗ прапорця `i`, тож `\p{Lu}` справді означає велику літеру.
 *     Саме це й називає коментар до {@link REPORTED_DIRECTION} як умову, за якої гілку можна
 *     повернути: «курс долара» не має великої літери після себе, «Курс Бородянка» має.
 *   * `6 на Бровари` — число, прийменник і велика літера поспіль. «3 будинки пошкоджено» не
 *     збігається: між числом і назвою стоїть іменник, а не `на`.
 *   * `знову на Андрушівку` — повторний захід, той самий зміст, що й у сусіднього індикатора, лише
 *     в коротшій формі, якої його перелік не знав.
 *   * `увага з Заходу` — вказівка, звідки саме йде ціль, а не заклик до уваги взагалі.
 *   * `на <Місто>` — найширша гілка, і єдина, що має другий замок: {@link TELEGRAPHIC_MOTION}.
 *
 * Точність вимірюється: `src/domain/classifier-gold.test.ts` тримає підлогу precision на 191
...
const contextIndicators: Array<{ name: string; pattern: RegExp; threatTypes: ThreatType[]; requires?: RegExp }> = [
  ...
  {
    // Телеграфне націлювання, гілки з власним структурним замком. Див. {@link TELEGRAPHIC_TARGETING}.
    name: 'телеграфне націлювання',
    pattern: TELEGRAPHIC_TARGETING,
    threatTypes: []
  },
  {
    // Та сама форма, найширша гілка — і тому з другим замком на рух уперед у часі.
    name: 'ціль на місто без названого типу',
    pattern: TELEGRAPHIC_APPROACH,
    threatTypes: [],
    requires: TELEGRAPHIC_MOTION
  }
];
...
/**
 * Загроза, названа разом із точкою пуску по той бік кордону.
 *
 * «📡 Триває загроза балістики з Брянська + загроза Цирконів з Курська» — повідомлення ПРО НАС, у
 * якому жодного українського місця немає, бо його ще немає й у реальності: ціль не вилетіла. Для
 * {@link FOREIGN_PLACE_PATTERN} воно виглядає точнісінько як репортаж про Росію, і сторож гасив
 * його разом зі справжніми репортажами — тобто гасив найраніше попередження, яке взагалі буває.
 *
 * Розрізняє їх порядок слів: слово про загрозу або пуск, а далі, в межах тієї самої фрази,
 * прийменник походження перед чужою назвою. «Над Курськом працює ППО» не має ні першого, ні
 * другого; «пуски з акваторії» вже має власний індикатор і сюди не потрапляє.
 */
const FOREIGN_ORIGIN_THREAT = new RegExp(
  `(?<!\\p{L})(?:загроз|пуск|запуск|старт|зліт|злет)[а-яіїєґ]*[^.!?\\n]{0,40}?(?<!\\p{L})з[іс]?\\s+(?:${FOREIGN_PLACE_STEMS.join('|')})`,
  'iu'
);
...
export function classifyMessage(text: string, locations: LocationLexeme[]): ClassifiedMessage {
  ...
  const aimedAtUkraine = matchedIndicators.some((indicator) => indicator.scope === 'launch')
    ...
    // Третя підстава: загроза, названа разом із місцем, ЗВІДКИ вона йде. «Триває загроза балістики
    // з Брянська + загроза Цирконів з Курська» — це заява про нас, а чуже місто в ній стоїть у
    // родовому відмінку після «з», тобто є точкою пуску, а не ціллю. Без цієї гілки
    // сторож іноземних місць гасив найранніше попередження, яке взагалі існує, — те, що приходить
    // до пуску.
    //
    // Замок — порядок слів, а не тема: спершу слово про загрозу чи пуск, і лише потім, у межах
    // тієї самої фрази, прийменник «з» перед чужою назвою. Випадок, заради якого сторож і
    // написаний, — «над Курськом працює ППО» — цієї форми не має: у ньому немає ні слова про
    // загрозу, ні прийменника походження.
    || FOREIGN_ORIGIN_THREAT.test(text);
  ...
  // визнали її спрямованою на Україну, вона мусить мати й обсяг. Інакше повідомлення проходить
  // сторожа й падає на `no_location` — тобто зникає тим самим способом, лише пізніше.
  const nationalScope = found.length === 0
    && (assertedIndicators.length > 0 || NATIONAL_SCOPE_CUES.test(text) || FOREIGN_ORIGIN_THREAT.test(text));
