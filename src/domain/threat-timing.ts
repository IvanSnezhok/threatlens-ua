...
// ------------------------------------------------------------------------------------------------
// Час, яким його бачить модель
// ------------------------------------------------------------------------------------------------
//
// Рішення власника 20.08.2026: у кожному запиті до моделі про повідомлення мають бути ТРИ часові
// величини — котра зараз година, о котрій канал це написав, і скільки повідомлення пролежало між
// цими двома моментами. До того модель бачила лише перше, і це робило неможливим саме те рішення,
// яке від неї вимагається: `timing` («зараз» чи «очікується») і `probability` визначаються тим, як
// давно джерело це сказало, а не тим, коли ми до цього дійшли.
//
// Дві функції нижче живуть тут, а не в жодному з двох класифікаторів, бо їх викликають обидва
// (`src/services/codex-classifier.ts` і `src/services/shadow-classifier.ts`), а тінь порівнюють з
// основною моделлю: два різні написання того самого часу зробили б розбіжність між ними
// нечитабельною. Часовий пояс — параметр, як і в `expectedWindow` вище: модуль лишається чистим.

/** Момент київським календарем, тими самими словами, якими його читає людина: «середа, 20.08.2026, 23:41». */
export function momentIn(at: Date, timeZone: string): string {
...
/**
 * Вік повідомлення словами: «3 хв тому», «2 год 10 хв тому».
 *
 * Готовий рядок, а не два ISO-моменти на відняття. Модель читає краще, ніж рахує, а помилка саме в
 * цьому відніманні коштує найдорожче: «пуски», прочитані через дві години після публікації, з
 * `timing: now` стають попередженням про те, що вже скінчилося.
 *
 * Мітка часу джерела попереду нашого годинника — не помилка й не привід кидати виняток: канали
 * ставлять час своїм годинником, і кілька секунд розходження є щодня.
 */
export function describeAge(publishedAt: Date, now: Date): string {
  const minutes = Math.round((now.getTime() - publishedAt.getTime()) / 60_000);
  if (minutes < 0) return 'щойно (мітка часу джерела попереду нашого годинника)';
  if (minutes < 1) return 'менше хвилини тому';
  if (minutes < 60) return `${minutes} хв тому`;
  const hours = Math.floor(minutes / 60);
  const rest = minutes % 60;
  return `${hours} год${rest ? ` ${rest} хв` : ''} тому`;
}
