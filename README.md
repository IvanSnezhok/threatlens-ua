...
| `AERIAL_MIRROR_ENABLED=false` | Stop reading the community aerial-alert mirror. **On by default** — it is the only HTTP alert source that needs no token, so it is what carries live alerts while the API applications above are pending. Turning it off stops the source being read; it does not withdraw alerts it was holding, see [`docs/OPERATIONS.md`](docs/OPERATIONS.md). |
| `AERIAL_MIRROR_URL` | Default `https://ubilling.net.ua/aerialalerts/`. Point it at a different mirror only if the payload shape matches — `{cachedat, states:{"<Область>":{alertnow,changed}}}` for the aggregated feed, `{cachedat, raw:[{regionType,regionName,activeAlerts}]}` for the passthrough below. |
| `AERIAL_MIRROR_RAW_SOURCE` | Default `ual`. Which upstream to read through the mirror's `?source=<x>&raw` passthrough, which returns that upstream's native body and is what gives this source **raion and hromada** granularity instead of oblast-only — Ukraine Alarm is the only probed upstream that publishes below oblast level. **Set it empty** to switch the passthrough off and restore the original oblast-only behaviour; that is the intended retreat if the upstream reshapes its body. |
| `AERIAL_MIRROR_REQUEST_GAP_MS` | Default 600. Gap between the raw poll and the aggregated cross-check on the polls that need one (the steady state is one request). The endpoint allows two requests per second per host and answers a burst with a truncated body, so the two are sequenced rather than fired together. |
...
| `ALERT_POLL_INTERVAL_SECONDS` | Default 4, accepted range 2–60. The gap between two passes of an alert-state leg, and the front of the end-to-end «upstream changed → shown as started» budget. It is a request gap, not a period: the next pass is armed when the previous finishes, so a slow upstream is never answered with a burst. **Per-provider floors clamp it and cannot be configured away** — the mirror 3 s (its documented cache TTL, under a 2 rps/host limit), alerts.in.ua 7 s (8.6 req/min against a 12/min hard limit), Ukraine Alarm 15 s (that API documents no limit at all, and an undocumented budget is not one to spend against). |
...
- `/api/v1/vectors` and `/api/v1/threats/:id/vector` — the chain of **reported** observations behind a threat: which source named which place, when, how strongly the movement between two places was attested (`basis`), and which threat class was reported as moving (`threatType`, on the envelope from the event and on every segment from the message that made that leg). Not a trajectory and not a forecast; the extrapolation of a chain is operator-only and is served by neither of these endpoints. The payload only ever grows: `threatType` was added without renaming, retyping or removing anything, and a client written against the earlier shape keeps working unchanged.
...
- `/api/v1/analytics/attack-stats` and `/api/v1/analytics/attack-stats/:regionId` — attack statistics and Poisson probabilities per region (24 oblasts and Kyiv), collected by a model with web search from **open sources** — not from the archive above — and recomputed deterministically: episodes with sources, days with attacks, night and ballistic shares, hourly comb, intervals, λ, p = 1 − e^(−λ), the ±σ scenarios and a day-by-day forecast calendar with `low|medium|high` levels. `POST …/:regionId/requests` records that a reader selected a region and, when the operator allows it, queues one bounded run. Off by default (`/ops` switch «Статистика ударів і ймовірності»); every payload carries the disclaimer, and nothing here touches alerts, events, risk or the map. Cached (`max-age=60` overview, `max-age=300` report).
...
| **How it ends** | Only an official source ends an alert. A polled source that goes quiet must stay quiet for `ALERT_END_DEBOUNCE_SECONDS` (60s = four missed polls of the slowest leg, more of the faster ones) before the alert drops — one incomplete API response cannot produce a false all-clear. A channel source announces its all-clear explicitly and is not debounced. |
...
| **Icon catalogue** | Ten weapon-class glyphs in four tones — consequence, confirmed, reported, analytic — ranked by danger, evidence, relation and freshness; three slots per territory and a `+N` badge for what ranking cut. The stack is a small point source that is re-emitted when it changes, kept separate from the polygon layers so the feature-state claim above stays true of the fills. No class glyph is an arrow: a glyph sits on a territory, and an arrow on a territory would assert a predicted target. |
| **Threat vectors** | Chains of *reported* observations, drawn as three line styles by basis — solid where one message stated the movement, dashed for a stated heading, dotted where the ordering is ours. A leg whose basis is `reported_transit` or `reported_direction` also carries an **arrowhead** at its head, rotated to the leg's on-screen bearing: those two are the grades where a source asserted movement, so the arrow repeats the source instead of adding to it. A leg whose basis is `observation_sequence` never gets one — «порядок наш; рух не стверджувало жодне джерело» — and keeps the dotted line and its node numbers. Each chain also shows one class chip at its newest drawn point, from the same icon catalogue above, so the map says what is moving and not only where. The reported-direction lines of `threat_events.geometry` get the same hollow arrowhead and the same chip. |
...
| | Why it matters |
|---|---|
| **Conditional forward warning** | The nightly digest is unconditional and fixed at 23:20. A warning issued *because* a threat is building — for the coming evening or day — does not exist yet. What does exist since migration 048 is the opposite kind of forward statement: a **base-rate** probability of an attack day per region, from open sources, delivered in the same digest and on the attacks page under a disclaimer — a statistic, not a warning. |
